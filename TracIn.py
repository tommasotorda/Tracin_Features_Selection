...
search_dir = "/home/tordatom/Dati_Imaging/BraTs_19/Segmentation2D/gradients/train_dice1/"
os.chdir(search_dir)
...
np.save("/home/tordatom/Dati_Imaging/BraTs_19/Segmentation2D/Radiomic_Features/score_dice1.npy", b) 

b = np.load("/home/tordatom/Dati_Imaging/BraTs_19/Segmentation2D/Radiomic_Features/score_dice1.npy",allow_pickle='TRUE').item()
...
proponents_index = [1410,633,774,1955,356]
opponents_index = [1949,1735,1543,1795,1728]


# In[ ]:


for index in proponents_index:
    index = int(index)
    channel = next(itertools.islice(train_dataset, map_train[index], map_train[index]+1))[1]
    seg = next(itertools.islice(train_dataset, map_train[index], map_train[index]+1))[2]
    plt.figure(figsize=(12, 8))
    ...
    plt.subplot(133)
    plt.title("Pred Segmentation")
    plt.imshow(model.predict(channel)[0,:,:,3])
    plt.savefig(f"/home/tordatom/Dati_Imaging/BraTs_19/Segmentation2D/Tracin_Images/dice3/proponents_of_test_70/dice3_train_proponent_{index}")
    plt.show()
...
for i in range(70,110, 10):
    ...
    plt.title("Pred Segmentation")
    plt.imshow(model.predict(channel)[0,:,:,3])
    plt.savefig(f"/home/tordatom/Dati_Imaging/BraTs_19/Segmentation2D/Tracin_Images/test/dice3_test_example_{i}")
    plt.show()
...
for index in opponents_index: 
    ...
    plt.title("Pred Segmentation")
    plt.imshow(model.predict(channel)[0,:,:,3])
    plt.savefig(f"/home/tordatom/Dati_Imaging/BraTs_19/Segmentation2D/Tracin_Images/dice3/opponents_of_test_100/dice3_train_opponents_{index}")
    plt.show()
